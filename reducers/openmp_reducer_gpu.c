 ...
 * OpenMP GPU offloading. Checksum: 499495.
 * Runtimes were collected on a gpu_a100 JLSE machine.
 * Kernel runtime: 13.091ms.
 */
...
int main(int argc, char **argv) {
    ...
    #pragma omp target data map(to: h[0:N])
    {
        ...
        #pragma omp target teams distribute parallel for reduction(+:result) map(result)
        for (unsigned i = 0; i < N; i++) {
            result += h[i];
	    printf("%u\n", result);
        } 
